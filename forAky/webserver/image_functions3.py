...
def get_prob(percent):
    #percent = 2*percent - 1

    if percent<min_percent: percent = -1 + (min_percent - percent)/min_percent
    else:                   percent = (percent - min_percent)/(1-min_percent)

    percent = percent*scale_tanh
    percent = np.tanh(percent)

    percent = (percent+1)/2

    return percent

def analyse_image_by_name(img_name):
    actual_name = img_name.split(".")[0]
    rn_name = actual_name.split("_")[0]

    rn_model = get_model(rn_name)

    source_path = "static/images/resources/" + img_name
    ...
    percent = get_prob(poz/(total-blanks))

    prob = round(percent*100,2)

    percent = str(prob)

    percent = percent.replace(".","-")

    destination_path = "static/images/resources/" + actual_name + "_" + percent + ".png"

    generate_image(image_patches,rez,destination_path)

    return prob, destination_path
...
def analyse_img_name(img_name):
    return run_in_separate_process(analyse_image_by_name,img_name)
